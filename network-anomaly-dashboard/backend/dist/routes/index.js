...
const networkMonitor = new NetworkMonitor_1.NetworkMonitor();
const anomalyService = new AnomalyDetectionService_1.AnomalyDetectionService(process.env.ML_MODEL_URL || 'http://localhost:5000');
...
// ML model status
router.get('/anomaly/model-status', async (req, res) => {
    try {
    ...
    try {
        ...
        const nodes = [
            ...
            ...adapters.slice(1, 4).map((adapter, index) => ({
                ...
                ip: adapter.address,
                type: 'server',
                status: adapter.status === 'Connected' ? 'normal' : 'suspicious',
